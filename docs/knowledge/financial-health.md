# Financial Health Scoring (Knowledge Reference)

<!--
SYNC PROVENANCE
  Source: Town skill `financial-health`
  ...
  Pilot: first skill in the Town -> Hermes mapping pipeline. See Town Doc
         "Town -> Hermes Skill Mapping Plan (June 2026)".
  NOTE: This is reference knowledge a SOUL.md may cite. It does not execute,
        mutate data, or write to .learnings/.
-->

## Purpose

Score a biotech company's financial survivability to produce a normalized 0-100 financial health score. This reference encodes the exact rules from Wake Robin's Module 2 pipeline (v1 + v2), the Dilution Risk Engine, Liquidity Scoring, and Short Interest Engine so that scoring is reproducible and auditable.
...
> Fix applied 2026-05-16 (Code Review L8): the Decimal mandate applies to scoring arithmetic only. Statistical analysis and transcendental functions (exp, log) may use float with explicit conversion back to Decimal before re-entering scoring paths.
...
> Fix applied 2026-05-16 (Code Review M1): Added explicit deceleration bonus formula.
...
> Fix applied 2026-05-16 (Code Review M2): Added zero-guard for avg_daily_volume and share_price (halted/zero-priced securities previously caused ZeroDivisionError).
...
> WARNING (W1, Doc Review 2026-05-17): Only V3 Default has a fully specified weight vector summing to 100%. The complete weight vectors for the other three sets need to be documented from production code (`module_5_composite.py`) or the deployed weight artifact.
