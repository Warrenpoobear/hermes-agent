# Clinical Scoring (Knowledge Reference)

<!--
SYNC PROVENANCE
  Source: Town skill `clinical-scoring`
  ...
  Pilot batch: second skill in the Town -> Hermes mapping pipeline (after financial-health, PR #35).
  NOTE: Reference knowledge a SOUL.md may cite. It does not execute, mutate data, or write to .learnings/.
-->

## Purpose

Score a biotech company's clinical development program to produce a normalized 0-100 clinical score. This reference encodes the exact rules, thresholds, and lookup tables from Wake Robin's pipeline (Module 4 + PoS Engine) so scoring is reproducible and auditable.
...
- Holds sole authority for clinical scoring overrides, indication mapping corrections (ticker_overrides_v3), and PoS benchmark adjustments.
- When indication_mapper.py returns confidence < 0.65, the operator should be consulted for manual classification before scoring proceeds.
...
> Note: Used by the PoS Engine (Step 5) to set baseline LOA context. NOT used in the Module 4 clinical composite (Step 14). For the Module 4 phase score, see Step 3.
...
Based on days since last trial update (PIT date field priority: `first_posted` > `last_update_posted` > `source_date` > `collected_at`).
...
> Fix applied 2026-05-16 (Code Review H5): Added explicit clamp to [0, 20]. Previously unbounded — many strong endpoints could exceed the stated range and push raw_total > 120 before the final clamp masked it.
