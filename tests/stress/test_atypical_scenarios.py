...
def _(home, kb):
    ...
    try:
        ...
        assert kb.get_task(conn, leaf).status == "todo", (
            "leaf should still be todo with B unfinished, got "
        ...
        assert kb.get_task(conn, leaf).status == "ready", (
            "leaf should promote with both parents done, got "
            f"{kb.get_task(conn, leaf).status}"
        )
...
def _(home, kb):
    ...
    try:
        ...
        result = kb.dispatch_once(conn, spawn_fn=lambda *_: 99999)  # noqa: F841
...
def _(home, kb):
    ...
    try:
        ...
        if len(ctx) > 200_000:
            print("  ⚠ build_worker_context unbounded on retry-heavy tasks "
